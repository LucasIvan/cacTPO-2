...
class Inventario:
    ...
    def agregar_producto(self, codigo, descripcion, cantidad, precio):
        ...
        if producto_existente:
            return jsonify(
                {'message': 'Ya existe un producto con ese código.'}), 400
    ...
    def agregar(self, codigo, cantidad, inventario):
        ...
        for item in self.items:
            if item.codigo == codigo:
                ...
                self.conexion.commit()
                return jsonify(
                    {'message': 'Producto agregado al carrito correctamente.'}), 200
        ...
        for item in self.items:
            if item.codigo == codigo:
                if cantidad > item.cantidad:
                    return jsonify(
                        {'message': 'Cantidad a quitar mayor a la cantidad en el carrito.'}), 400
                item.cantidad -= cantidad
                if item.cantidad == 0:
                ...
                self.conexion.commit()
                return jsonify(
                    {'message': 'Producto quitado del carrito correctamente.'}), 200
        return jsonify(
            {'message': 'El producto no se encuentra en el carrito.'}), 404
